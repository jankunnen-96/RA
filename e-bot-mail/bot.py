...
PERSONAL_NOTIFY_EMAIL = os.environ.get("PERSONAL_NOTIFY_EMAIL", "jan.kunnen@hotmail.com")
...
def send_email(subject, body, recipients):
    ...
    try:
        ...
        print(f"Sent notification to {recipients}", flush=True)
    except Exception as e:
        print(f"Error sending notification: {e}", flush=True)


def send_acceptance_notification(subject, case_url):
    recipients = list(dict.fromkeys([NOTIFY_EMAIL, PERSONAL_NOTIFY_EMAIL]))
    body = (
        f"Een keuringsaanvraag werd automatisch aanvaard.\n\n"
        f"Aanvraag: {subject}\n"
        f"Link: {case_url}\n"
    )
    send_email(f"Aanvraag automatisch aanvaard: {subject}", body, recipients)


def send_missed_notification(subject, response):
    body = (
        f"Een keuringsaanvraag kon niet automatisch aanvaard worden.\n\n"
        f"Aanvraag: {subject}\n"
        f"Status code: {response.get('status_code', 'onbekend')}\n"
        f"Pagina: {response.get('page_title', 'onbekend')}\n"
        f"Link: {response.get('final_url', 'onbekend')}\n"
        f"Fout: {response.get('error', '-')}\n"
    )
    send_email(f"Aanvraag GEMIST: {subject}", body, [PERSONAL_NOTIFY_EMAIL])
